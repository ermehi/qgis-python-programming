"""
6.4.1.1	Añadir nombres de capas vectoriales y raster de la ToC de QGIS en los desplegables
...
def load_vectors(self):
    ...
    """
    self.dlg.comboBox_vectorLayer.clear()
    self.dlg.comboBox_vectorLayer.addItem(self.tr("--- Select vector layer ---"))
    for layer in QgsProject.instance().mapLayers().values():
        if layer.type() == QgsMapLayer.VectorLayer:
            if layer.geometryType() == QgsWkbTypes.PointGeometry:
                self.dlg.comboBox_vectorLayer.addItem(layer.name())
...
def load_rasters(self):
    ...
    self.dlg.comboBox_rasterLayer.addItem(self.tr("--- Select raster layer ---"))
    for layer in QgsProject.instance().mapLayers().values():
        if layer.type() == QgsMapLayer.RasterLayer:
            self.dlg.comboBox_rasterLayer.addItem(layer.name())
...
"""
6.4.1.2	Botones de acción abrir capas vectoriales y raster existentes
...
self.dlg.toolButton_searchVectorLayer.clicked.connect(self.open_vector)
self.dlg.toolButton_searchRasterLayer.clicked.connect(self.open_raster)

"""
6.4.1.3	Botón de acción para seleccionar shapefile de salida
...
def search_path_output_shp(self):
    ...
    """
    str_path_output_shp, str_filter = \
        QFileDialog.getSaveFileName(caption=self.tr("Search path output shapefile"),
                                    filter="Shapefiles (*.shp)")
    if len(str_path_output_shp) > 0:
        self.dlg.lineEdit_pathOutputShp.setText(str_path_output_shp)
    else:
        self.dlg.lineEdit_pathOutputShp.clear()

# b) añadir signal/slot connection en el metodo run()
self.dlg.toolButton_searchPathOutputShp.clicked.connect(self.search_path_output_shp)

"""
6.4.1.4	Controlando el comportamiento de opciones de salida
...
def push_memory_layer(self):
    """
    Disabled shp QLineEdit & QToolButton when QRadioButton memory layer is clicked
    """
...
def push_rb_shp(self):
    """
    Enabled shp QLineEdit & QToolButton when QRadioButton shp is clicked
    """
...
"""
6.4.2	Operaciones de análisis espacial
10_AnalilsisEspacial ***************************************************************************************************
"""
# C:\dev_plugins\query_raster_from_point\query_raster_from_point.py

# a) Crear signal/slot connection tootButton process en el método run
self.dlg.toolButton_process.clicked.connect(self.process)

# 6.4.2.1	Obtención de la información necesaria para el procesamiento
...
def get_vector_layer(self):
    ...
    """
    layer = None
    layername = self.dlg.comboBox_vectorLayer.currentText()
...
def get_raster_layer(self):
    ...
    """
    layer = None
    layername = self.dlg.comboBox_rasterLayer.currentText()
    for lyr in QgsProject.instance().mapLayers().values():
        if lyr.name() == layername:
            layer = lyr
            break
...
def get_ui_parameters(self):
    ...
    if self.dlg.comboBox_vectorLayer.currentIndex() == 0:
        # TODO: msg
        return
    self.qgs_vector_layer = self.get_vector_layer()
    # get QgsRasterLayer from comboBox
    if self.dlg.comboBox_rasterLayer.currentIndex() == 0:
        # TODO: msg
        return
    self.qgs_raster_layer = self.get_raster_layer()
    # get path output shapefile from lineEdit
    if self.dlg.radioButton_outputShapefile.isChecked():
        self.str_path_output_shp = self.dlg.lineEdit_pathOutputShp.text()
        if len(self.str_path_output_shp) == 0:
            # TODO: msg
            return

# 6.4.2.2	Implementación del procesamiento

# e) añadir nuevas classes necesarias para el procesamiento en la seccion de imports
# import PyQt5 classes
from qgis.core import (QgsProject, QgsMapLayer, QgsWkbTypes,
                       QgsField, QgsVectorLayer, QgsRaster, QgsVectorFileWriter)
...
def get_raster_values_from_vector_layer(self):
    """
    Brief: Obtains raster layer values from point vector layer.
    Create memory layer or shapefile with source layer fields,
    adding new field with obtained raster value.
    Finally, add layer result in QGIS
    """
    ...
    str_name_memory_layer = "New memory layer"  # TODO: name memory layer, pass to constant
    ...
    # gets raster value of each point in vector layer and updates corresponding attribute in target ml
    ...
    if self.dlg.radioButton_outputMemoryLayer.isChecked():
...
"""
6.4.3.4	Eliminación de resultados de procesamientos anteriores
...
def process(self):
    ...
    """
    if self.get_ui_parameters():
        if self.dlg.checkBox_removePreviousProcessing.isChecked():
...
"""
6.4.3.6	Mejoras del procesamiento
...
def process(self):
    ...
    if self.get_ui_parameters():
        ...
        if self.get_raster_values_from_vector_layer():
            self.iface.messageBar().pushMessage(APPLICATION_NAME + " v." + c.APPLICATION_VERSION,
                                                self.tr('Processing completed successfully'),
                                                Qgis.MessageLevel.Info,
                                                10)

"""
6.4.3.7	Internacionalización
17_Traduccion **********************************************************************************************************
"""
...
def process(self):
    ...
    Controls processing execution
    """
    tag_messagelog = self.tr("Query Raster from Point process")
    if self.get_ui_parameters():
        QgsMessageLog.logMessage(self.tr("Collected parameters required for processing"), tag_messagelog, Qgis.Info)
        if self.dlg.checkBox_removePreviousProcessing.isChecked():
            self.remove_previous_memory_layer()
            QgsMessageLog.logMessage(self.tr("Removed previous proccessings"), tag_messagelog, Qgis.Info)
        if self.get_raster_values_from_vector_layer():
            QgsMessageLog.logMessage(self.tr("Spatial analysis completed"), tag_messagelog, Qgis.Info)
            self.iface.messageBar().pushMessage(APPLICATION_NAME + " v." + APPLICATION_VERSION,
                                                self.tr('Processing completed successfully'),
                                                Qgis.MessageLevel.Info,
                                                10)
